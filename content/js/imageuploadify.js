...
;(function($, window, document, undefined) {
  ...
  $.fn.imageuploadify = function(opts) {
    ...
    this.each(function() {
      ...
      <button type="button" class="btn uploadBtn"><span>
          או בחר קבצים להעלאה
      </span></button>
      </div>
      </div>
      ...
      const retrieveFiles = (files) => {
        ...
        for(let index = 0; index < files.length; ++index) {
          if (!accept || compareMimeType(result, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])) {
            ...
            _.find(totalFiles, function(file,i) {
              if (file.file.name == files[index].name){
                exist = true;
              }
            });
            if (!exist) {
            ...
            /*else{
              Materialize.toast('הקובץ כבר קיים', 4000) // 4000 is the duration of the toast
            } */
          }
        }
      }
      ...
      const readingFile = (id, file) => {
        ...
        let container = $(`<div class='imageuploadify-container'>
            <button type='button' class='btn btn-danger' > <i class="material-icons prefix"> cancel</i></button>
